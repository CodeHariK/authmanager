...
export function PasskeyManagement({ passkeys }: { passkeys: Passkey[] }) {
  ...
  async function handleDeletePasskey(passkeyId: string) {
    ...
    if (result?.error) {
      return { success: false, error: result.error.message || "Failed to delete passkey" }
    }
            ...
            <Card key={passkey.id}>
              <CardHeader className="flex gap-2 items-center justify-between">
                ...
                <ActionButtonWithConfirm
                  ...
                  dialogTitle="Delete Passkey"
                  dialogDescription={`Are you sure you want to delete "${passkey.name}"? This action cannot be undone.`}
                  confirmText="Delete"
